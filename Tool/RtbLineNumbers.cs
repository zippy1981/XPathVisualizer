...
//
// slightly modified, to remove dependency on the Microsoft.VisualBasic namespace.
//
// Wed, 31 Mar 2010  17:17
//
...
namespace LineNumbers
{
    ...
    public class RtbLineNumbers : System.Windows.Forms.Control
    {
        ...
        private RichTextBox zParent {
            ...
            set {
                if (withEventsField_zParent != null) {
                    withEventsField_zParent.LocationChanged -= zParent_LocationChanged;
                    withEventsField_zParent.Move -= zParent_Move;
                    withEventsField_zParent.Resize -= zParent_Resize;
                    withEventsField_zParent.DockChanged -= zParent_DockChanged;
                    withEventsField_zParent.TextChanged -= zParent_TextChanged;
                    withEventsField_zParent.MultilineChanged -= zParent_MultilineChanged;
        ...
        /// <summary>
        /// This Sub determines which textlines are visible in the
        /// ParentRichTextBox, and makes LineNumberItems (LineNumber +
        /// ItemRectangle) for each visible line. They are put into the
        /// zLNIs List that will be used by the OnPaint event to draw the
        /// LineNumberItems.
        /// </summary>
        /// <remarks></remarks>
        private void Update_VisibleLineNumberItems()
        {
            ...
            // Make sure the LineNumbers are aligning to the same height as
            // the zParent textlines by converting to screencoordinates and
            // using that as an offset that gets added to the points for the
            // LineNumberItems.
            zPointInParent = zParent.PointToScreen(zParent.ClientRectangle.Location);
            zPointInMe = this.PointToScreen(new Point(0, 0));

            // zParentInMe is the vertical offset to make the
            // LineNumberItems line up with the textlines in zParent.
            zParentInMe = zPointInParent.Y - zPointInMe.Y + 1;

            // The first visible LineNumber may not be the first visible
            // line of text in the RTB if the LineNumbercontrol's .Top is
            // lower on the form than the .Top of the parent
            // RichTextBox. Therefor, zPointInParent will now be used to
            // find zPointInMe's equivalent height in zParent, which is
            // needed to find the best StartIndex later on.
            zPointInParent = zParent.PointToClient(zPointInMe);

            // --- NOTES:
            // Additional complication is the fact that when wordwrap is
            // enabled on the RTB, the wordwrapped text spills into the
            // RTB.Lines collection, so we need to split the text into lines
            // ourselves, and use the Index of each zSplit-line's first
            // character instead of the RTB's.
            string[] zSplit = txt.Split("\r\n".ToCharArray());
            ...
            else
            {
                ...
                // Store only those LineNumberItems for lines that are visible.
                ...
                // zStartIndex now holds the index of a character in the
                // first visible line from zParent.Text .  Now it will be
                // pointed at the first character of that line (chr(10) =
                // Linefeed part of the vbCrLf constant)
                zStartIndex = Math.Max(0, Math.Min(txt.Length - 1, txt.Substring(0, zStartIndex).LastIndexOf('\n') + 1));

                // We now need to find out which zSplit-line that character
                // is in, by counting the newlines that come
                // before it.
                zSplitStartLine = Math.Max(0, CountNewlines(txt.Substring(0, zStartIndex)) - 1);

                // zStartIndex starts off pointing at the first character of
                // the first visible line, and will be then be pointed to
                ...
                for (zA = zSplitStartLine; zA <= zSplit.Length - 1; zA++)
                {
                    ...
                    // For performance reasons, the list of LineNumberItems
                    // (zLNIs) is first built with only the location of its
                    // itemrectangle being used. The height of those
                    // rectangles will be computed afterwards by comparing
                    // the items' Y coordinates.
                    ...
                    {
                        // The more lines there are in the RTB, the slower
                        // the RTB's .GetPositionFromCharIndex() method
                        // becomes.  To avoid those delays from interfering
                        // with the scrolling speed, this speedbased exit for
                        // is applied (0.05 sec) zLNIs will have at least 1
                        // item, and if that's the only one, then change its
                        // location to 0,0 to make it readable.
                ...
                if (zA < zSplit.Length) {
                    ...
                    zPoint = zParent.GetPositionFromCharIndex(Math.Min(zStartIndex, txt.Length - 1));
        ...
        private void zParent_LocationChanged(object sender, System.EventArgs e)
        {
            this.Refresh();
            this.Invalidate();
        }

        private void zParent_Move(object sender, System.EventArgs e)
        {
            this.Refresh();
            this.Invalidate();
        }

        private void zParent_Resize(object sender, System.EventArgs e)
        {
            this.Refresh();
            this.Invalidate();
        }

        private void zParent_DockChanged(object sender, System.EventArgs e)
        {
            this.Refresh();
            this.Invalidate();
        }

        private void zParent_TextChanged(object sender, System.EventArgs e)
        {
            //this.Refresh();
            //this.Invalidate();
        }

        private void zParent_MultilineChanged(object sender, System.EventArgs e)
        {
            this.Refresh();
            this.Invalidate();
        }
